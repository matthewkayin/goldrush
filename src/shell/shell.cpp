...
static const int SHELL_BUTTON_X = SCREEN_WIDTH - 132 + 14; 
static const int SHELL_BUTTON_Y = SCREEN_HEIGHT - MATCH_SHELL_UI_HEIGHT + 10;
static const int SHELL_BUTTON_PADDING_X = 32 + 4;
static const int SHELL_BUTTON_PADDING_Y = 32 + 6;
...
MatchShellState* match_shell_init(int lcg_seed, Noise* noise) {
    ...
    // Init match
    match_init(state->match_state, lcg_seed, players, (MatchInitMapParams) {
...
void match_shell_update(MatchShellState* state) {
    ...
    if (state->replay_mode) {
        ...
        ui_begin_column(state->replay_ui, ivec2(BUTTON_PANEL_RECT.x + 8, BUTTON_PANEL_RECT.y + 4), 2);
            ui_element_size(state->replay_ui, ivec2(0, 16));
            ui_begin_row(state->replay_ui, ivec2(0, 0), 4);
                ui_element_position(state->replay_ui, ivec2(0, 2));
            ...
            ui_begin_row(state->replay_ui, ivec2(0, 0), 6);
                    ...
                    if (state->move_animation.name == ANIMATION_UI_MOVE_CELL && cell_layer == CELL_LAYER_GROUND) {
                        ...
                        if (entity_index != INDEX_INVALID) {
                            ...
                            if (remembered_entity_index != MATCH_ENTITY_NOT_REMEMBERED) {
                                ...
                                render_sprite_frame(match_shell_get_entity_select_ring(remembered_entity.type, state->move_animation.name == ANIMATION_UI_MOVE_ATTACK_ENTITY), ivec2(0, 0), entity_center_position, RENDER_SPRITE_CENTERED, 0);
                            }
                        }
                    }
                }
    ...
    // Entities
    {
        ...
        for (uint8_t team = 0; team < MAX_PLAYERS; team++) {
            ...
            for (uint32_t remembered_entity_index = 0; remembered_entity_index < state->match_state.remembered_entities[team].size(); remembered_entity_index++) {
                ...
                ivec2 params_position = (remembered_entity.cell * TILE_SIZE) - state->camera_offset;
                RenderSpriteParams params = (RenderSpriteParams) {
