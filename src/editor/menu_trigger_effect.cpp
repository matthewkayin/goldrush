...
void editor_menu_trigger_effect_set_effect_type(EditorMenuTriggerEffect& menu, TriggerActionType effect_type);
...
EditorMenuTriggerEffect editor_menu_trigger_effect_open(const TriggerAction& effect, uint32_t effect_index) {
    ...
    for (uint32_t effect_type = 0; effect_type < TRIGGER_ACTION_TYPE_COUNT; effect_type++) {
        menu.effect_type_items.push_back(trigger_effect_type_str((TriggerActionType)effect_type));
    }

    if (effect.type == TRIGGER_ACTION_TYPE_HINT) {
...
void editor_menu_trigger_effect_update(EditorMenuTriggerEffect& menu, UI& ui, EditorMenuMode& mode) {
    ...
    ui_begin_column(ui, ivec2(MENU_RECT.x + 8, MENU_RECT.y + 30), 4);
        ...
        if (editor_menu_dropdown(ui, "Effect Type:", &effect_type, menu.effect_type_items, MENU_RECT)) {
            editor_menu_trigger_effect_set_effect_type(menu, (TriggerActionType)effect_type);
            if (menu.effect.type == TRIGGER_ACTION_TYPE_HINT) {
    ...
    if (mode == EDITOR_MENU_MODE_SUBMIT) {
        if (menu.effect.type == TRIGGER_ACTION_TYPE_HINT) {
            strncpy(menu.effect.hint.message, menu.hint_value.c_str(), TRIGGER_ACTION_HINT_MESSAGE_BUFFER_LENGTH - 1);
        }
    }
}
