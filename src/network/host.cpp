...
#endif
    log_info("Created host with backend %u", backend);
...
#ifdef GOLD_STEAM
    if (host->backend == NETWORK_BACKEND_STEAM) {
        log_info("Connecting to host with steam identity %s", connection_info.steam.identity_str);
        ...
        log_debug("Host has new peer with connection %u. Peer count %u", host->steam.peers[host->steam.peer_count], host->steam.peer_count);
    }
#endif
...
uint16_t network_host_get_peer_count(NetworkHost* host) {
    if (host->backend == NETWORK_BACKEND_LAN) {
        return (uint16_t)host->lan.host->peerCount;
    }
#ifdef GOLD_STEAM
    if (host->backend == NETWORK_BACKEND_STEAM) {
