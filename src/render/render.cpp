...
SDL_Surface* render_load_font(FontName name) {
    ...
    if (ttf_font == NULL) {
        log_error("Unable to open font %s: %s", path, SDL_GetError());
        return NULL;
    }
    ...
    for (int glyph_index = 0; glyph_index < FONT_GLYPH_COUNT; glyph_index++) {
        ...
        if (glyphs[glyph_index] == NULL) {
            log_error("Error rendering glyph %s for font %s: %s", text, path, SDL_GetError());
            return NULL;
        }
...
bool render_load_sprites() {
    ...
    for (int tileset = 0; tileset < TILESET_COUNT; tileset++) {
        ...
        if (tileset_surfaces[tileset] == NULL) {
            log_error("Unable to load tileset %s: %s", tileset_path, SDL_GetError());
            return false;
        }
    }
    ...
    for (int sprite = 0; sprite < SPRITE_COUNT; sprite++) {
        ...
        if (params.strategy == SPRITE_IMPORT_TILE) {
            ...
            if (sprite_surface == NULL) {
                log_error("Unable to load sprite %s: %s", sprite_path, SDL_GetError());
                return false;
            }
    ...
    SDL_Surface* atlas_surface = SDL_CreateSurface(ATLAS_WIDTH, ATLAS_HEIGHT, SDL_PIXELFORMAT_ABGR8888);
    ...
    glGenTextures(1, &state.sprite_texture);
    glBindTexture(GL_TEXTURE_2D, state.sprite_texture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, ATLAS_WIDTH, ATLAS_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, atlas_surface->pixels);
